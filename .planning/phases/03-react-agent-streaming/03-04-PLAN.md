...
<task type="auto">
  ...
  <action>
Update `backend/main.py` to add the streaming agent endpoint. The existing Phase 2 endpoint remains for backwards compatibility but the new endpoint handles streaming.
...
- Old retrieval endpoint moved to /api/retrieve
