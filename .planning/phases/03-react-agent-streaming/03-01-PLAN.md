...
must_haves:
  ...
  artifacts:
    ...
    - path: "backend/agent/__init__.py"
      provides: "Module exports for state and tools"
      exports: ["AgentState", "search_knowledge_base"]
  key_links:
...
<objective>
Create the foundational components for the ReAct agent: state definition and RAG tool.

Purpose: Establish the building blocks that the LangGraph state machine will use. The RAG tool wraps the existing retriever with proper tool descriptions so the agent knows when to invoke it.

Output: `backend/agent/` module with state.py, tools.py (prompts.py created in Plan 03-02)
</objective>
...
<task type="auto">
  ...
  <action>
Create `backend/agent/__init__.py` with initial exports (will add more after graph.py in Plan 03-03):

```python
from .state import AgentState
from .tools import search_knowledge_base

__all__ = ["AgentState", "search_knowledge_base"]
```
